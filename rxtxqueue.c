...
#include "cpu.h"       // for parse_cpu_list(), parse_cpu_mask()
...
static const struct usage_opt usage_options[] = {
  ...
  {'d', "DIRECTION", "Capture only packets matching DIRECTION. DIRECTION can"
                       ...
                       " '" RXSELF "', 'tx' when '" TXSELF "', and 'rxtx' when"
                                                          " '" RXTXSELF "')."},
  {'f', "FILE", "Write packet queue count to output file"},
...
/* ========================================================================= */
int num_queues(const char *ifname, pcap_direction_t direction) {
  int status = 0;
  char *path = NULL;

  status = asprintf(&path, "/sys/class/net/%s/queues/", ifname);
  if (status == -1) {
    return -1;
  }
  ...
  int rx = 0;
  int tx = 0;
  if (direction == PCAP_D_INOUT || direction == PCAP_D_IN) {
    rx = sysfs_count(path, "rx-");
  }

  if (direction == PCAP_D_INOUT || direction == PCAP_D_OUT) {
    tx = sysfs_count(path, "tx-");
  }
...
int main(int argc, char **argv) {
  ...
  struct bpf_insn prog[] = {
    /*
     * PACKET_FANOUT_EBPF_QUEUE - queue aligned PACKET_FANOUT_QM (i.e. aligned
     *                            rxq to fd mappings).
     *
     ...
     */
    { 0x61, 0x3, 0x1, 0x0004, 0x00000000 }, // r3 = *(u32 *)(r1 + 4)
    { 0xb7, 0x2, 0x0, 0x0000, 0x00000001 }, // r2 = 1
    { 0x55, 0x3, 0x0, 0x0001, 0x00000004 }, // if r3 != 4 goto 1
    { 0xb7, 0x2, 0x0, 0x0000, 0x00000000 }, // r2 = 0
    { 0x61, 0x0, 0x1, 0x000c, 0x00000000 }, // r0 = *(u32 *)(r1 + 12)
    { 0x1f, 0x0, 0x2, 0x0000, 0x00000000 }, // r0 -= r2
